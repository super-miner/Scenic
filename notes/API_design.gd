...
## Throws an error and only does first transition
GlobalSceneManager.queue_set_scene(test_scene)
GlobalSceneManager.with_transition(&"Fade")
...
GlobalSceneManager.preload(test_scene, current_scene) # Setting the owner to the current scene so that when it goes away test_scene gets freed
# ... (some code)
if ...:
	GlobalSceneManager.queue_set_scene(test_scene)
	GlobalSceneManager.with_transition(&"Fade")
...
## Does nothing and does not block scene loading
GlobalSceneManager.queue_set_scene(test_scene)
